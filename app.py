...
# models = {}
# for file in os.listdir("model_pkl"):
#     if file.endswith(".pkl"):
#         model_name = file.replace(".pkl", "")
#         models[model_name] = joblib.load(os.path.join("model_pkl", file))
#
...
for root, _, files in os.walk(base_folder):
    for file in files:
        ...
        rel = os.path.relpath(full_path, base_folder).replace("\\", "/")
        model_key = rel[:-4]
        try:
            ...
            try:
                models[model_key] = joblib.load(full_path)
            except Exception as e:
                print(f"❌ Could not load '{model_key}': {e}")


popular_df = models.get("books_model_pkl/popular")
books = models.get("books_model_pkl/books")
pt = models.get("books_model_pkl/pt")
similarity_score = models.get("books_model_pkl/similarity_score")


movies = models.get("movies_model_pkl/new_movies_df")
movies_similarity = models.get("movies_model_pkl/similarity")


music_dict = models.get("musics_model_pkl/music_recommendation_binary")
music = pd.DataFrame(music_dict) if music_dict is not None else None
similarity = models.get("musics_model_pkl/similarities_binary")

svm = models.get("medicines_model_pkl/medicine_df")
